...
class GdgListFragment : Fragment() {
    ...
    private val viewModel: VM_GdgList by lazy { ViewModelProvider(this)[VM_GdgList::class.java] }
    lateinit var binding: FragmentGdgListBinding

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = FragmentGdgListBinding.inflate(inflater)
        ...
        viewModel.regionList.observe(viewLifecycleOwner, Observer {
            ...
            val inflatedChip = LayoutInflater.from(chipGroup.context)

            val children = it.map { regionName ->
                val chip = inflatedChip.inflate(R.layout.region, chipGroup, false) as Chip
